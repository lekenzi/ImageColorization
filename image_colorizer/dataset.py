...
import numpy as np
import torch
from fastai.data.external import URLs, untar_data  # type: ignore
...
class ColorizationDataLoader:
    ...
    def _prepare_paths(self):
        ...
        paths_subset = np.random.choice(
            paths, self.num_images, replace=False
        )  # choosing images randomly
        rand_idxs = np.random.permutation(self.num_images)
        train_idxs = rand_idxs[
            : int(self.num_images * 0.8)
        ]  # choosing the first 80% as training set
        val_idxs = rand_idxs[
            int(self.num_images * 0.2) :
        ]  # choosing last 20% as validation set
    ...
    def get_dataloaders(self):
        train_dataset = self.ColorizationDataset(
            paths=self.train_paths, split="train", size=self.size
        )
        val_dataset = self.ColorizationDataset(
            paths=self.val_paths, split="val", size=self.size
        )
